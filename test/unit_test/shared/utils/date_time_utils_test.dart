...
void main() {
  ...
  group('daysBetween', () {
    test('when from < to', () {
      ...
      final result = DateTimeUtils.daysBetween(from: from, to: to);
  ...
  group('timezoneOffset', () {
    test('when timezone is UTC', () {
      withClock(Clock.fixed(DateTime.utc(2023, 1, 1, 0, 0, 0, 0, 0)), () {
        final result = DateTimeUtils.timezoneOffset();
        expect(result, 0);
      });
    });
  });
  ...
  group('utcToLocal', () {
    test('when utcTimestampMillis is 0', () {
      final result = DateTimeUtils.utcToLocal(0);
      expect(
        result,
        isA<DateTime>().having(
    ...
    if (DateTimeUtils.timezoneOffset() == 7) {
      test('when timezone is +07:00 and utcDateTime is DateTime(2024, 12, 31, 7, 15, 30)', () {
        ...
        final result = DateTimeUtils.utcToLocal(input);

        expect(result, expected);
      });
    }

    if (DateTimeUtils.timezoneOffset() == 9) {
  ...
  group('localToUtc', () {
    test('when localTimestampMillis is 0', () {
      final result = DateTimeUtils.localToUtc(0);
      expect(
        result,
        isA<DateTime>().having(
          (e) => e.year == 1970 && e.month == 1 && e.day == 1 && e.minute == 0 && e.second == 0,
    ...
    if (DateTimeUtils.timezoneOffset() == 7) {
      test('when timezone is +07:00 and localDateTime is DateTime(2024, 12, 31, 14, 15, 30)', () {
        ...
        final result = DateTimeUtils.localToUtc(input);
  ...
  group('tryParse', () {
    test('when dateTime is null', () {
      final result = DateTimeUtils.tryParse(null);
      expect(result, null);
    });

    test('when format is null', () {
      final result = DateTimeUtils.tryParse('2023-01-01');
      expect(result, DateTime(2023, 1, 1));
    });

    test('when format is null and dateTime is invalid', () {
      final result = DateTimeUtils.tryParse('');
      expect(result, null);
    });

    test('when format is not null', () {
      final result =
          DateTimeUtils.tryParse('2023--01--01 04:06:08', format: 'yyyy--MM--dd hh:mm:ss');
    ...
    test(
      ...
      () {
        final result = DateTimeUtils.tryParse('', format: 'yyyy-MM-dd', utc: false);
        expect(result, null);
      },
    );
  });
